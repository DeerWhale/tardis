...
class InnerVelocitySolverWorkflow(SimpleTARDISWorkflow):
    ...
    def __init__(self, configuration, mean_optical_depth="rosseland", tau=None):
        ...
        self.iterations_values = {'t_radiative':[], 'dilution_factor':[], 't_inner':[], 'v_inner_boundary':[], "mean_optical_depth": np.full((self.total_iterations, self.simulation_state.no_of_shells), np.nan), 'mask':[]}
    ...
    def estimate_v_inner(self):
        ...
        self.iterations_values["mean_optical_depth"][self.completed_iterations,self.simulation_state.geometry.v_inner_boundary_index:] = tau_integ[self.simulation_state.geometry.v_inner_boundary_index:]
